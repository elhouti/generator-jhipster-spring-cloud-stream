...
module.exports = class extends BaseGenerator {
  ...
  prompting() {
      ...
      function isNotValidBootstrapServerString(input) {
          return input.split(',').some(bootstrapServer => boostrapServersUnitPattern.test(bootstrapServer) === false);
      }
      ...
      //PROMPTING FOR KAFKA
      {
          when: response => response.messageBrokerType === KAFKA && !this.options['skip-prompts'],
          type: 'confirm',
          name: 'cleanup',
          message: 'Do you want to clean up your current Kafka configuration (if already exist)?',
          default: false
      },
      {
          when: response => response.messageBrokerType === KAFKA && !this.options['skip-prompts'],
        ...
        this.prompt(prompts).then((props) => {
            ...
            if (this.props.messageBrokerType === RABBITMQ) {
              ...
              if (this.props.cleanup) {
                  this.kafka_props.cleanup = this.props.cleanup;
              }
              if (this.props.bootstrapServers) {
                  this.kafka_props.bootstrapServers = this.props.bootstrapServers;
              }
            }
            done();
        });
  ...
  writing() {
      ...
      switch (this.messageBrokerType) {
        case RABBITMQ:
            this._private_installRabbitMq();
            break;
        case KAFKA:
            files.writeFiles(this);
            this._private_installKafka();
            break;
        default:
            break;
      }
